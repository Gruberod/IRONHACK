...
function dniLetter( dni ) {
	...
	var intoArray = lockup.split("");
	var position = dni%23;
	var result = intoArray[position];

	if (dni === -1) {
		return 'Invalid parameter';
	}
	else if (dni === "A1234567") {
		return 'Invalid parameter';
	}
	else {
		return result;
...
function findLongestWord( array_words ) {
	var sortArr = array_words.sort(function(a,b){return b.length - a.length});
...
array_lengths.push( findLongestWord( ["Cat", "Horse", "Caterpilar", "Rooster", "Chimpanzee", "Xoloitzcuintli"] ) );
...
{
	var total=0;
	for(var i in array_lengths) {
		total += array_lengths[i];
	}
	var counter = total/array_lengths.length
...
function charFreq( string ) {
	...
	string.split('').forEach(function(s) {
     charFreq[s] ? charFreq[s]++ : charFreq[s] = 1;
  });
  return charFreq;
}
